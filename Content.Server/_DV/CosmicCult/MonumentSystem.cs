...
public sealed partial class MonumentSystem : SharedMonumentSystem
{
    ...
    private const string BlueAlert = "blue";
    private const string OctarineAlert = "octarine";

    private HashSet<Entity<HumanoidAppearanceComponent>> _targets = [];
    ...
    {
        if (!HasComp<HumanoidAppearanceComponent>(args.User) // Humanoids only!
    ...
    private void OnFinaleStartDoAfter(Entity<MonumentComponent> ent, ref StartFinaleDoAfterEvent args)
    {
        ...
        if (_station.GetOwningStation(ent) is { } station)
            _alert.SetLevel(station, OctarineAlert, true, true, true, true);
        ...
        _evac.CancelRoundEndCountdown(args.User, false);
    ...
    private void OnFinaleCancelDoAfter(Entity<MonumentComponent> ent, ref CancelFinaleDoAfterEvent args)
    {
        ...
        if (_station.GetOwningStation(ent) is { } station)
            _alert.SetLevel(station, BlueAlert, true, true, true); // Blue makes more sense than green IMO.
